...
FROM @xml.nodes('/Errores/error') AS T(error);
GO
...
-- EMPLEADOS Y MOVIMIENTOS - FECHA POR FECHA

DECLARE @xmlEmpleados   XML
DECLARE @xmlMovimientos XML

SET @xmlEmpleados = '
<Empleados>
    <empleado Puesto="Camarero"      ValorDocumentoIdentidad="6993943" Nombre="Kaitlyn Jensen"   FechaContratacion="2013-12-17"/>
    ...
    <empleado Puesto="Niñera"        ValorDocumentoIdentidad="3389054" Nombre="Sofia Herrera"    FechaContratacion="2022-08-09"/>
</Empleados>'
...
IF OBJECT_ID('tempdb..#MovimientosTemp') IS NOT NULL
    DROP TABLE #MovimientosTemp;
...
CREATE TABLE #MovimientosTemp (
    ValorDocId    VARCHAR(20),
    NombreTipoMov VARCHAR(100),
    Fecha         DATE,
    Monto         DECIMAL(10,2),
    PostByUser    VARCHAR(100),
    PostInIP      VARCHAR(50),
    PostTime      DATETIME
);
...
SELECT
    Empleado.value('@Puesto',                  'VARCHAR(100)'),
    Empleado.value('@ValorDocumentoIdentidad', 'VARCHAR(20)'),
    Empleado.value('@Nombre',                  'VARCHAR(150)'),
    Empleado.value('@FechaContratacion',       'DATE')
FROM @xmlEmpleados.nodes('/Empleados/empleado') AS T(Empleado);

INSERT INTO #MovimientosTemp
SELECT
    movimiento.value('@ValorDocId',       'VARCHAR(20)'),
    movimiento.value('@IdTipoMovimiento', 'VARCHAR(100)'),
    movimiento.value('@Fecha',            'DATE'),
    movimiento.value('@Monto',            'DECIMAL(10,2)'),
    movimiento.value('@PostByUser',       'VARCHAR(100)'),
    movimiento.value('@PostInIP',         'VARCHAR(50)'),
    movimiento.value('@PostTime',         'DATETIME')
FROM @xmlMovimientos.nodes('/Movimientos/movimiento') AS T(movimiento);

-- Obtener todas las fechas unicas en orden
DECLARE @TodasFechas TABLE (Fecha DATE);

INSERT INTO @TodasFechas
SELECT Fecha FROM (
    SELECT DISTINCT FechaContratacion AS Fecha FROM #EmpleadosTemp
    UNION
    SELECT DISTINCT Fecha FROM #MovimientosTemp
) AS Fechas
ORDER BY Fecha ASC;


DECLARE @FechaLoop     DATE;
DECLARE @ValorDocId    VARCHAR(20);
DECLARE @NombreTipoMov VARCHAR(100);
DECLARE @Fecha         DATE;
DECLARE @Monto         DECIMAL(10,2);
DECLARE @PostByUser    VARCHAR(100);
DECLARE @PostInIP      VARCHAR(50);
...
SELECT @FechaLoop = MIN(Fecha) FROM @TodasFechas;

WHILE @FechaLoop IS NOT NULL
BEGIN

    INSERT INTO Empleado (IdPuesto, ValorDocumentoIdentidad, Nombre, FechaContratacion)
    SELECT
        (SELECT Id FROM Puesto WHERE Nombre = ET.NombrePuesto),
        ET.ValorDocId,
        ET.Nombre,
        ET.FechaContratacion
    FROM #EmpleadosTemp ET
    WHERE ET.FechaContratacion = @FechaLoop;


    DECLARE movCursor CURSOR FOR
        SELECT ValorDocId, NombreTipoMov, Fecha, Monto, PostByUser, PostInIP, PostTime
        FROM #MovimientosTemp
        WHERE Fecha = @FechaLoop
        ORDER BY PostTime ASC;

    OPEN movCursor;
    FETCH NEXT FROM movCursor INTO
        @ValorDocId, @NombreTipoMov, @Fecha, @Monto,
        @PostByUser, @PostInIP, @PostTime;
    ...
    BEGIN
        ...
        FROM Empleado
        WHERE ValorDocumentoIdentidad = @ValorDocId AND EsActivo = 1;

        SELECT @IdTipoMov  = Id,
               @TipoAccion = TipoAccion
        FROM TipoMovimiento
        WHERE Nombre = @NombreTipoMov;
        ...
        BEGIN
            INSERT INTO Movimiento (
                IdEmpleado, IdTipoMovimiento, Fecha,
                Monto, NuevoSaldo, IdPostByUser, PostInIP, PostTime)
            VALUES (
                @IdEmpleado, @IdTipoMov, @Fecha,
                @Monto, @NuevoSaldo, @IdUsuario, @PostInIP, @PostTime);
    ...
    CLOSE movCursor;
    DEALLOCATE movCursor;

    SELECT @FechaLoop = MIN(Fecha) FROM @TodasFechas WHERE Fecha > @FechaLoop;
END
...
SELECT Id, Nombre, SaldoVacaciones FROM Empleado ORDER BY Nombre;
GO