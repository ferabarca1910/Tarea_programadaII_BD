...
SELECT
    (SELECT Id FROM Empleado WHERE ValorDocumentoIdentidad = Movimiento.value('@ValorDocId','VARCHAR(20)')),
    (SELECT Id FROM TipoMovimiento WHERE Nombre = Movimiento.value('@IdTipoMovimiento','VARCHAR(100)')),
    Movimiento.value('@Fecha','DATE'),
    Movimiento.value('@Monto','DECIMAL(10,2)'),
    0,
    (SELECT Id FROM Usuario WHERE Username = Movimiento.value('@PostByUser','VARCHAR(100)')),
    Movimiento.value('@PostInIP','VARCHAR(50)'),
    Movimiento.value('@PostTime','DATETIME')
FROM @xml.nodes('/Movimientos/movimiento') AS T(Movimiento);
